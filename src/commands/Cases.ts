...
export class CasesCommand extends Subcommand {
  ...
  public override async autocompleteRun(interaction: AutocompleteInteraction) {
    ...
    if (subcommand === 'join' || subcommand === 'close') {
      const openCases = (await Cache.getCases()).filter((x) => x.open && x.serverId === interaction.guildId);
      return interaction.respond(
        (await Promise.all(openCases.map(async (c) => ({ name: `${(await Cache.getUser(c.ownerId!))?.name}: ${c.id}`, value: c.id })))).slice(0, 25)
      );
    }
  }
  ...
  public async caseJoin(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const cs = await Cache.getCase(interaction.options.getString('caseid')!);
  ...
  public async caseFind(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const cUsr = await Cache.getUser(usr.id);
    ...
    const cases = (await Cache.getCases()).filter((x) => x.ownerId === cUsr.id && x.serverId === interaction.guildId);
