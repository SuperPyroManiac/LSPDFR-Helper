...
export class RPHAdvancedErrors {
  public static async processAdvancedErrors(log: RPHLog, rawLog: string): Promise<RPHLog> {
    //! Multi Session Check
    ...
    //! Combined Errors
    //* Missing Depends
    const err1 = (await Cache.getError(1))!.clone();
    for (const match of rawLog.matchAll(new RegExp(err1.pattern!, 'gm'))) {
      ...
      err1.pluginList.push((await Cache.getPlugin(match[2]!)) ?? new Plugin(match[2]));
    }
    ...
    if (rawLog.match(new RegExp('DamageTrackingFramework: \\[VERSION OUTDATED\\]', 'gm'))) log.outdated.push((await Cache.getPlugin('DamageTrackingFramework'))!);
